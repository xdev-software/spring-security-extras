...
public class DefaultOAuth2RefreshFilterAuthCheckMetrics implements OAuth2RefreshFilterAuthCheckMetrics
{
	private final Counter ignored;
	private final Counter noAuth;
	private final Map<OAuth2AuthChecker.AuthCheckOutcome, Counter> authCheckMetrics;
	
	public DefaultOAuth2RefreshFilterAuthCheckMetrics(final MeterRegistry meterRegistry)
	{
		final Function<String, Counter> builder = o -> meterRegistry.counter(
			SSESharedMetrics.PREFIX + "filter_auth_check",
			SSESharedMetrics.TAG_OUTCOME,
			o);
		
		this.ignored = builder.apply("ignored");
		this.noAuth = builder.apply("no_auth");
